...
final class ModelRouter: ObservableObject {
    ...
    @Published private(set) var currentBackend: ModelBackend = .deterministic
    ...
    private init() {
        ...
        self.currentModelId = deterministicModel.modelId
    }
    
    // MARK: - Model Selection
    
    /// Select the best available model for the given input
    /// Priority: Apple On-Device > Core ML > Deterministic
    private func selectModel(for input: ModelInput) -> (any OnDeviceModel, String?) {
        ...
        // Default: Deterministic template model (always available)
