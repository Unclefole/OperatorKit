...
// ============================================================================
// TOGETHER AI CLIENT — Meta Llama Cloud Client
//
// Together AI provides dedicated API access to Meta Llama models.
...
final class TogetherLlamaClient: @unchecked Sendable {

    static let shared = TogetherLlamaClient()
    ...
    private func resolveKey() throws -> String {
        ...
        if let vaultKey = try? APIKeyVault.shared.retrieveKeyString(for: .cloudLlama) {
            return vaultKey
        }
        // Fallback: environment variable (CI / Xcode scheme only)
        if let envKey = ProcessInfo.processInfo.environment["TOGETHER_API_KEY"], !envKey.isEmpty {
            return envKey
        }
        throw CloudModelError.apiKeyMissing(.cloudLlama)
    }
    ...
    /// Send a governed completion request to Together AI (Meta Llama).
    ...
    func generate(
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw CloudModelError.responseParseFailed("Invalid Together AI response structure")
        }
